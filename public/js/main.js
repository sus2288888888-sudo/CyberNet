...
function playNotificationSound(force = false) {
    if (!force && (isDeafened || userStatus === 'dnd')) return;
    ...
    try {
        ...
        osc.type = 'sine';
        osc.frequency.setValueAtTime(force ? 440 : 500, ctx.currentTime);
        osc.frequency.exponentialRampToValueAtTime(force ? 880 : 1000, ctx.currentTime + 0.1);

        gain.gain.setValueAtTime(force ? 0.1 : 0.05, ctx.currentTime);
        gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + (force ? 0.5 : 0.2));

        osc.start();
        osc.stop(ctx.currentTime + (force ? 0.5 : 0.2));
    } catch (e) { console.warn("Sound blocked by browser", e); }
}
